 ...
 */
 function typewrite()
 {
     let str="<我的小傻瓜上当啦<这可不是最后的礼物呦😜<试一试WZZLOVEDBBFOREVER这个啦<";//利用<作为换行符 -作为空格
     let strp="";
     let i=0;
     function print1()
     {
 ...
 function typewrite2()
 {
    ...
    var futruetime =  new Date('2021,4,23').getTime(); // 未来时间转换为时间戳
    ...
    let str="<嘿，我的宝贝你发现了最终的礼物，<*我们一起走过了："+day+"天"+hour+"小时"+minute+"分"+second+"秒"+"，^<这一年里我真的不止一回哭过/(ㄒoㄒ)/~~<我真的真的好想你呀，可是这话嘴边又不知该说些什么<所有的千言万语汇成一句<我爱你💕-我的宝贝<";//利用<作为换行符 -作为空格
    let strp="";
    let i=0;
    let flag = 0;
    function print1()
    {
        if(str[i]=='<')
        {
            document.getElementById("box2").innerHTML=strp+"<br><br>"+'|';
            strp+="<br><br>";
        }
        else
        if(str[i]=='-')
        {
            document.getElementById("box2").innerHTML=strp+'&nbsp&nbsp&nbsp';
            strp+="&nbsp&nbsp&nbsp";
        }
        else
        if(str[i]=='*')
        {
            document.getElementById("box2").innerHTML=strp+'<span id="elapseClock">';
            strp+="<span id='elapseClock'>";
        }
        else
        if(str[i]=='^')
        {
            document.getElementById("box2").innerHTML=strp+'</span>';
            strp+="</span>";
        }
        else
        {
            strp+=str[i];
            document.getElementById("box2").innerHTML=strp+'|';
        }
        i++;
    }
    function print2()
    {
        setTimeout(()=>{document.getElementById("box2").innerHTML=strp+'&nbsp';},100);
        setTimeout(()=>{document.getElementById("box2").innerHTML=strp+'|';},630);
    }
    ...
    function print3()
    {
        ...
        var minute = parseInt(time%3600/60); // 分 -(day*24) 以60秒为一整份 取余 剩下秒数 秒数/60 就是分钟数
        var second = parseInt(time%60);  // 以60秒为一整份 取余 剩下秒数
        setTimeout(()=>{document.getElementById("elapseClock").innerHTML="我们一起走过了："+day+"天"+hour+"小时"+minute+"分"+second+"秒，";},1000);
    }
    ...
    setTimeout(() => {
        id1=setInterval(print3)
        id2=setInterval(print2,1060);//注意setInterval内部直接写函数的时候是直接写函数名不加括号的。
    },(str.length-1)*90);
    //0.53秒闪一次
 }
...
let clickb=false;//判断播放按钮是否已经按过了，如果已经按过了才可以执行键盘事件
 ...
 let a=[
     ...
     {name:"E" ,yin:77, str:"<br><br><br><br>若是你决定了要来见我<br><br>请务必提前一周告诉我<br><br>我好将10200个小时前的相见再反复回想"},
     ...
     {name:"I" ,yin:33, str:"<br><br>我见过沧海的云，巫峡的雨。 <br>我见过一月的雪覆于白山，又渐变于葱茏。 <br>我在峨眉的林里云兴霞蔚， 一径之后，雾水成露，沾于衣襟。<br>我听过柔橹漂浮，声声入水，又归于沉寂。 <br>我看到春风八里十里，衣袖带花。 无论何时忆起， 它们实在是人生可喜， <br><br>但都不如我此刻遇到的你。"},
     ...
     {name:"L" ,yin:88, str:"<br><br><br><br>世界上美好的东西不太多,<br><br>立秋傍晚从河对岸吹来的风,<br><br>和二十来岁笑起来要人命的你。"},
     ...
     {name:"O" ,yin:77, str:"<br><br><br><br>我们或者在月光下闲游，<br><br>或者在灯光下谈心，<br><br>手握着手，心对着心，<br><br>就像一对五十岁的情人。"},
     ...
     {name:"U" ,yin:555, str:"<br><br>我把我整个的灵魂都给你，<br><br>连同它的怪癖，耍小脾气，忽明忽暗，<br><br>一千八百种坏毛病， 它真讨厌。只有一点好， 爱你"},
     {name:"V" ,yin:222, str:"<br><br><br><br>我一生都是坚定不移的唯物主义者<br><br>唯有你<br><br>我希望有来生"},
 ...
 function fl(e)
 {
     ...
     if((key<='z'&&key>='a')||(key<='Z'&&key>='A'))//避免大小写的错误
     {
         ...
         if(s.indexOf("ILOVEU")!=-1&&records==0)
         {
             window.onkeyup = null
             setTimeout(function()
             {
        ...
        //  WZZLOVEDBBFOREVER
         if(s.indexOf("WZZLOVEDBBFOREVER")!=-1&&(records==0||records==1))
         {
             window.onkeyup = null
             setTimeout(function()
             {
                 ...
                 let music=document.createElement("audio");
                 x.setAttribute("src", `./video/IDO.mp3`);
                 x.setAttribute("autoplay","autoplay");
                 x.setAttribute("loop","loop");
             },750);
             setTimeout(typewrite2,3000);//进行打字
         }
     }
     }  
 }
